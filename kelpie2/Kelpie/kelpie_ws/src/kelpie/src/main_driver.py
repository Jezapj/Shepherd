...
import numpy as np
import rospy
from kelpie_hardware_interface.current_sense.current_saver import current_listener
from kelpie_hardware_interface.current_sense.joint_saver import joint_listener
...
class KelpieDriver:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            rospy.loginfo(f"Estopped? {self.state.currently_estopped}")
            while self.state.currently_estopped == 0:
                ...
                rospy.loginfo(f"BattVol{overall_batt_voltage}")
                ...
                if self.state.currently_estopped == 0:
                    if not once:
                        ...
                        result_current = current_listener()
                        rospy.loginfo(f"Here is hi: {result_current}")
                        result_joint = joint_listener()
                        rospy.loginfo(f"Here is hi: {result_joint}")
                        command = self.input_interface.get_command(self.state, self.message_rate)
                        self.state.behavior_state = BehaviorState.REST
                        self.controller.run(command)
                        self.controller.publish_joint_space_command(self.state.joint_angles)
